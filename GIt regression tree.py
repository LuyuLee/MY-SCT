...
class DecisionTree(object):
    ...
    def cal_best_theta_value(self, ke, attri_list):  ### The atri_listt is the value belong to one feature [1] is the label
        ...
        for i in range(len(data) - 1):
            ...
            count = 0
            for c, j in enumerate(attri_list):  ### enumerate : both index and value:https://blog.csdn.net/liu_xzhen/article/details/79564455
                # print(c,'J is : ', j)
                if j[0] <= cur_theta:
                    # print("J[0] less", j[0])
    ...
    def create_decision_tree(self, dict_of_everything, class_val, eta_min_val):  # ,fea_list):
        if len(class_val) < eta_min_val:   ### if the number of the leaf < the minest number we having setting
            ...
            # print('bool_R: ', Node.is_leaf(rightchild))
            return root_node
