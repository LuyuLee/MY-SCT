...
def SelectFromModel(X, y):
    ...
    lsvc = LogisticRegressionCV(multi_class='ovr', cv=3, Cs=ListRound, penalty="l1", solver='liblinear')
...
def loadDataSet(FileName, Cv=True):
    data = []; label = []
    fr = open(FileName)
    for line in islice(fr, 1, None):
        lineArr = re.split(',|\n ', line.strip())
        # print('lineArr: ', lineArr[0])
        datainput = list(map(float, lineArr[4:]))
    ...
    if Cv:
        data_new,FeatureChoose = SelectFromModel(np.array(data), np.array(label))
...
def FitLinearModel (X, Y, Cv, lowV, highV):
    ...
    LgCV = LogisticRegressionCV(multi_class='ovr', cv=Cv, Cs=ListRound, penalty="l1", solver='liblinear')
...
def FindThePara():
    ...
    if Command.capitalize() == 'Yes':
        SaveModel(LogModel, 'LinearLogisticsModel')
    else:
        print('What a shame!') 
...
def CaculatePi(FileName):
    ...
    LogModel = FitLinearModel(X_train_Mat, Y_train_Mat, 3, -10, 3)
